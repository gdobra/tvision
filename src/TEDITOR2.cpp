                             ...
                             )
{
    ...
    if( newSize > bufLen + delCount )
        if ( setBufSize(ulong(newSize)) == False )
            {
    ...
    ulong selLines = countLines( &buffer[bufPtr(selStart)], selLen );
    if( curPtr == selEnd )
        {
        if( allowUndo == True )
            {
    ...
    ulong lines = countLines( &buffer[curPtr], length );
    curPtr += length;
    curPos.y += lines;
    drawLine = curPos.y;
...
Boolean TEditor::insertText( const void *text, ulong length, Boolean selectText )
{
...
ulong TEditor::lineMove( ulong p, int count )
{
    ulong i = p;
    p = lineStart(p);
    int pos = charPos(p, i);
...
ulong TEditor::nextLine( ulong p )
{
    return nextChar(lineEnd(p));
}

ulong TEditor::nextWord( ulong p )
{
...
ulong TEditor::prevLine( ulong p )
{
  return lineStart(prevChar(p));
}

ulong TEditor::prevWord( ulong p )
{
...
void TEditor::setBufLen( ulong length )
{
    bufLen = length;
...
Boolean TEditor::setBufSize( ulong newSize )
{
...
void TEditor::setCurPtr( ulong p, uchar selectMode )
{
    ulong anchor;
...
void TEditor::setSelect( ulong newStart, ulong newEnd, Boolean curStart )
{
    ulong p;
    if( curStart != 0 )
        p = newStart;
